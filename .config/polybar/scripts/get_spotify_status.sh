...
if [ -z "$(playerctl status 2>/dev/null)" ]; then
	...
	polybar-msg -p "$(pgrep -f "polybar player")" hook spotify-play-pause 2 2>&1 >/dev/null
	...
	polybar-msg -p "$(pgrep -f "polybar player")" hook spotify-play-pause 1 2>&1 >/dev/null
	playerctl --player=playerctld metadata --format "{{ title }} - {{ artist }}"
fi
